...
if mode == "Hugging Face API":
    st.sidebar.warning("⚠️ The free Hugging Face API no longer supports image-to-image tasks. To forcefully use your Layout Sketch, please switch to 'Local (Requires GPU/Colab)' Mode!")
    method = st.sidebar.radio("Generation Method", ["Baseline (Text-to-Image)"])
else:
    method = st.sidebar.radio("Generation Method", ["Baseline (Text-to-Image)", "Layout-Guided (Img2Img)"], index=1)
...
if mode == "Hugging Face API":
    ...
    # FLUX.1 quickly hallucinates layouts without Img2Img, but it generates stunning aesthetics.
    models.insert(0, "black-forest-labs/FLUX.1-schnell")
        
    model_id = st.sidebar.selectbox("Model", models)
else:
...
with col1:
    ...
    if uploaded_file is not None:
        try:
            ...
            # Generate Prompt
            auto_prompt = generate_prompt(layout_data)
            prompt = st.text_area("Generated Prompt (Edit below to add styles like 'photorealistic', '4k', etc.)", value=auto_prompt, height=100)
                ...
                if mode == "Hugging Face API" and not api_key:
                    ...
                    with st.spinner("Generating image..."):
                        try:
                            ...
                            vis_img.save("outputs/layout_sketch.png", format="PNG")
                            st.session_state['layout_sketch_path'] = "outputs/layout_sketch.png"
...
with col2:
    ...
    if 'generated_image' in st.session_state:
        if method == "Layout-Guided (Img2Img)" and 'layout_sketch' in st.session_state:
            ...
            with res_col1:
                st.image(st.session_state['layout_sketch'], caption="Reference Layout Sketch", width="stretch")
            with res_col2:
                st.image(st.session_state['generated_image'], caption="Final Layout-Guided Result", width="stretch")
        else:
            st.image(st.session_state['generated_image'], caption="Final Baseline Result", width="stretch")
